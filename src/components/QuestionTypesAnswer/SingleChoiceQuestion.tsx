    ...
    const tryPlay = () => {
      if (audioRef.current) {
        ...
        audioRef.current
          ...
          .catch(() => {
            if (!playedRef.current && retry < maxRetry) {
              retry++;
  ...
  useEffect(() => {
    if (question.question) {
      ttsApi
        ...
        .catch(() => {
          // Nếu lỗi gọi API thì bỏ qua TTS, hiển thị câu hỏi luôn
          setAudioReady(true);
        });
    }
  }, [question.number, question.question]);

  // Đọc đáp án đúng khi có đáp án đúng
  useEffect(() => {
    if (
      answerChecked &&
      ...
      ttsApi
        ...
        .catch(() => {
          // Nếu lỗi thì không phát audio, không ảnh hưởng giao diện
        });
    }
  }, [
    answerChecked &&
