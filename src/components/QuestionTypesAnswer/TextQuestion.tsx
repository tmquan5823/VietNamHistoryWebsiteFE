...
const TextQuestion: React.FC<TextQuestionProps> = ({
  ...
  const playAudioWithRetry = (url: string, maxRetry = 2, delay = 1200) => {
    if (lastPlayedUrlRef.current === url) return; // Đã phát rồi, không phát lại
  ...
  // TTS: Đọc câu hỏi khi hiện câu hỏi mới
  useEffect(() => {
    if (question.question) {
      ttsApi
        .getTts(question.question)
        .then((res) => {
          if (res.data?.async) {
  ...
  // TTS: Đọc đáp án đúng khi có đáp án đúng
  useEffect(() => {
    if (
      answerChecked &&
      answerChecked.correct_answer &&
      ...
      const correctText = `Đáp án đúng là: ${answerChecked.correct_answer.join(
        ", "
      )}`;
      ttsApi.getTts(correctText).then((res) => {
        if (res.data?.async) {
          playAudioWithRetry(res.data.async);
        }
      });
    ...
    answerChecked &&
      answerChecked.correct_answer &&
      answerChecked.correct_answer.join(","),
  ]);

  // Hiệu ứng hiện từng chữ cho câu hỏi trong vòng 2 giây, chỉ bắt đầu khi audioReady
  useEffect(() => {
    if (question.question && audioReady) {
      let i = 0;
  ...
  }, [question.number, question.question, audioReady]);
  ...
  return (
    ...
    >
      ...
      <audio
        ...
        onError={() => {
          // Nếu lỗi, sẽ tự động retry trong playAudioWithRetry
        }}
      />
    </div>
  );
