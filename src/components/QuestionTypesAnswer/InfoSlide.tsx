...
const InfoSlide: React.FC<InfoSlideProps> = ({
  ...
  // Chỉ bắt đầu hiệu ứng hiện từng chữ khi audio đã sẵn sàng
  useEffect(() => {
    if (question.info && audioReady) {
      let i = 0;
        ...
        audioRef.current
          ...
          .catch(() => {
            if (!played && retry < maxRetry) {
              ...
              setAudioReady(true); // Cho phép hiển thị info nếu audio lỗi
            }
          });
      }
    };
  ...
  // TTS: Đọc nội dung info khi hiển thị
  useEffect(() => {
    if (question.info) {
      ttsApi
        .getTts(question.info)
        .then((res) => {
          if (res.data?.async) {
