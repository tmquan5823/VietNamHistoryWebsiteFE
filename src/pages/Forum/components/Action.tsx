...
type ActionProps = {
  ...
  topic: string;
  myPostsOnly: boolean;
  onMyPostsOnlyChange: (value: boolean) => void;
};
...
function Action({
  ...
  myPostsOnly,
  onMyPostsOnlyChange,
}: Omit<ActionProps, "status" | "handleStatusChange"> & {
  myPostsOnly: boolean;
  onMyPostsOnlyChange: (value: boolean) => void;
}) {
  const topicOptions = [
    ...
    <aside className="w-full md:w-full flex-shrink-0">
      ...
      <div className="bg-neutral-100 rounded-lg p-4 mb-4">
        ...
        <div className="flex items-center gap-2 mb-2 mt-2">
          <input
            id="my-posts-checkbox"
            type="checkbox"
            checked={myPostsOnly}
            onChange={(e) => onMyPostsOnlyChange(e.target.checked)}
            className="accent-[#D12827]"
          />
          <label
            htmlFor="my-posts-checkbox"
            className="text-sm font-semibold text-[#5D4037] cursor-pointer"
          >
            Chỉ hiện bài viết của tôi
          </label>
        </div>
        <button
          type="button"
