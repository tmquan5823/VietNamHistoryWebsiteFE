...
export const QuizSetForm: React.FC<{
  topics: Topic[];
  updateQuizSetWithQuestions: (quiz: QuizSetWithQuestionsParams) => void;
  quiz: QuizSetWithQuestionsParams;
}> = ({ topics, updateQuizSetWithQuestions, quiz }) => {
      ...
      <Form {...methods}>
        ...
        >
          ...
          <div className="mb-6">
            ...
            />
            {thumbnail && (
              <div className="mt-3 rounded-xl overflow-hidden border-2 border-gray-200 shadow w-full max-h-40 flex justify-center items-center bg-gray-50 relative">
                <button
                  type="button"
                  onClick={() => setThumbnail(null)}
                  className="absolute top-2 right-2 bg-white bg-opacity-80 hover:bg-opacity-100 text-gray-700 rounded-full w-7 h-7 flex items-center justify-center shadow-md z-10"
                  title="Xóa ảnh"
                >
                  &times;
                </button>
